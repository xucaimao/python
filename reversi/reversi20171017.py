...
# 20171016实现了每局比赛的比分、下棋记录的动态显示
# 更改程序逻辑层级，用户只接触interface层
# 20171020增加了一个列表，用于存储下棋记录的每一步的surf,后期可以扩充滚动条
# 20171020增加电脑对战功能
...
class Reversi:
    ...
    def __init__(self, stpiece):
        self.who2who = 1              # 对战状态，1-人机对战 2-人人对战 3-机机对战
...
class Interface:
    ...
    def __init__(self,myreversi):
        ...
        self.surflist = []                                  # 背景、棋子等surf的列表
        self.recordsurflist=[]                              # 下棋记录文字surf的列表
        self.roundscoresurflist=[]                          # 每局比分文字surf的列表
        self.msgsurf = pygame.image.load("msgbox.png")
        self.passsurf = pygame.image.load("pass.png")
        self.startsurf = pygame.image.load("startmenu2.png")
    ...
    def selectPlayer(self):
        ...
        if choice==2:
            ...
            # 人人对战
            self.reversi.who2who = 1
            self.reversi.player[0] = 'people'
            self.reversi.player[1] = 'people'
            self.reversi.playername[0] = '玩家一'
            self.reversi.playername[1] = '玩家二'
        else:
            self.reversi.who2who = 3
            self.reversi.player[0] = 'machine'
            self.reversi.player[1] = 'machine'
            self.reversi.playername[0] = '电脑一'
            self.reversi.playername[1] = '电脑二'
    ...
    def addPieceToDisplay(self,position):
        # 定义一个surf列表，不用每次都重新生成文字surf，提高速度
        # 白棋用灰色显示，黑棋用黑色显示
        # 显示格式为A1，与数据存储格式[row,col]刚好相反；同时需要注意[row,col]是从[0,0]为起点
        # 此程序可以修改一下，存储记录的文本文件
        # 考虑到黑棋先行，因此，在记录列表中，偶数下标的值对应的总是黑棋，奇数下标对应的总是白棋
        msg = "第" + str(self.reversi.step) + "步 : "
        if position[0] == -1 or position[1] == -1:
            msg += "PASS"
        else:
            msg += chr(position[1] + ord('A')) + " , " + str(position[0] + 1)
        if len(self.reversi.record) % 2 == 1:
            msgcolor = (0, 0, 0)
        else:
            msgcolor = (150, 150, 150)
        font = pygame.font.Font("FZLBFW.ttf", 18)  # 数字大小
        msgsurf = font.render(msg, 1, msgcolor)
        self.recordsurflist.append(msgsurf)

    def removePieceFromDisplay(self):
        while len(self.recordsurflist)>len(self.reversi.record):
            self.recordsurflist.pop()

    def drawRecord(self,surflist,position):
        #把列表surflist中最后的十二步,显示在position
        reclen=len(surflist)
        if reclen==0:
            return
        msgposx,msgposy=position
        if reclen>=12:
            p=reclen-12
        else:
            p=0
        i=0
        while p<=reclen-1:
            self.win.blit(surflist[p], [msgposx, msgposy+i*25])
            i+=1
            p+=1

    def addScoreToDisplay(self):
        #把比分变成surf加入列表，黑棋胜用黑色表示，白棋胜用灰色表示，平局用红色表示
        #显示格式为黑:白
        reclen=len(self.reversi.roundscore)
        if reclen==0:
            return
        font = pygame.font.Font("FZLBFW.ttf", 18)  # 数字大小
        score=self.reversi.roundscore[reclen-1]
        msg="第"+str(reclen)+"局 "+ str(score[0]) +" : "+str(score[1])
        if score[0]>score[1]:
            #本局黑棋胜，字用黑色
            msgcolor=(0,0,0)
            ...
            msgcolor=(150,150,150)
        msgsurf = font.render(msg, 1, msgcolor)
        self.roundscoresurflist.append(msgsurf)
    ...
    def drawScreenAddPiece(self, moveorpass, bpos):
        ...
        # 显示总比分
        scorestr2 = str(self.reversi.totalscore[0]) + " : " + str(self.reversi.totalscore[1])
        ...
        #显示下棋记录
        self.drawRecord(self.recordsurflist,(765,270))
        self.drawRecord(self.roundscoresurflist,(640,270))
        pygame.display.flip()
    ...
    def getPlayerInput(self):
        ...
        if self.reversi.currentEnableMove():
            ...
            if self.reversi.currentPlayer() == 'people':
                ...
                #是电脑，返回计算结果
                if self.reversi.who2who==3:
                    if self.reversi.current_piece==-1:
                        #黑棋的智能程序
                        return myreversi.getComputerMove()
                    else:
                        #白棋的智能程序
                        return myreversi.getComputerMove2()
    ...
    def judgeWinLose(self):
        return self.reversi.judgeWinLose()
...
roundover = False
while not roundover:
    totalscore = myinterface.judgeWinLose()
    if totalscore == False:
        # 没有胜负，继续比赛
        roundover = False
        ...
        # 有胜负，显示对话框，一局结束
        if myreversi.who2who != 3:
            myinterface.drawMsgBox(totalscore)
        myinterface.resetRound()
        myinterface.addScoreToDisplay()
        myinterface.drawScreenAddPiece(False, [-1, -1])
        if myinterface.reversi.who2who==3 and myinterface.reversi.round>50:
            roundover=True
        else:
            roundover = False
